...
public class GuruhController {
   ...
   if(b) return "Guruh mavjud";
       ...
       // fakultetdagi id teng malumot olib keladi
       guruh.setFakultet(fakultetRepozitary.findById(guruhDTO.getFakultetID()).get());
       List<Fanlar> fanlarList=new ArrayList<>();
       // bunda fanlar id malumotlarni oz ichiga oladi
       for(Integer d: guruhDTO.getFanID()){
           fanlarList.add(fanlarRepozitary.findById(d).get());
       }
       guruh.setFanlarList(fanlarList);
       List<Talaba>talabaList=new ArrayList<>();
       for(Integer g: guruhDTO.getTalabaID()){
           talabaList.add(talabaRepository.findById(g).get());
       }
